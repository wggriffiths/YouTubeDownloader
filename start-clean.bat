...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference='SilentlyContinue'; $url='https://github.com/wggriffiths/YouTubeDownloader/releases/latest/download/ytdl-windows-x64.exe'; Write-Host '[*] Downloading...'; Invoke-WebRequest -Uri $url -OutFile '%PPATH%ytdl.exe' -UseBasicParsing; if (Test-Path '%PPATH%ytdl.exe') { Write-Host '[+] Downloaded ytdl.exe' } else { exit 1 }"
